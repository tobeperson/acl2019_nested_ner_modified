...
os.environ["CUDA_VISIBLE_DEVICES"] = "4" #设置可见的GPU编号
config = tf.ConfigProto()
config.gpu_options.allow_growth = True  #允许GPU占用率动态增长
session = tf.Session(config=config)

class Network:
    def __init__(self, threads, seed=42):
        # Create an empty graph an  d a session
        graph = tf.Graph() #创建图
        graph.seed = seed  #设置随机数种子
        self.session = tf.Session(graph = graph, config=tf.ConfigProto(inter_op_parallelism_threads=threads,  #打开对话，设置图
    ...
    def construct(self, args, num_forms, num_form_chars, num_lemmas, num_lemma_chars, num_pos,
                  ...
                  predict_only):
        with self.session.graph.as_default():  #打开默认图

            # Inputs 定义输入容器
            self.sentence_lens = tf.placeholder(tf.int32, [None], name="sentence_lens")  #句子长度
            self.form_ids = tf.placeholder(tf.int32, [None, None], name="form_ids")       #格式ID
            self.lemma_ids = tf.placeholder(tf.int32, [None, None], name="lemma_ids")     #词干ID
            self.pos_ids = tf.placeholder(tf.int32, [None, None], name="pos_ids")        #位置ID
            self.pretrained_form_wes = tf.placeholder(tf.float32, [None, None, pretrained_form_we_dim], name="pretrained_form_wes")  #格式词嵌入矩阵
            self.pretrained_lemma_wes = tf.placeholder(tf.float32, [None, None, pretrained_lemma_we_dim], name="pretrained_lemma_wes")  #词干词嵌入矩阵
            self.pretrained_fasttext_wes = tf.placeholder(tf.float32, [None, None, pretrained_fasttext_dim], name="fasttext_wes")
            self.pretrained_bert_wes = tf.placeholder(tf.float32, [None, None, pretrained_bert_dim], name="bert_wes")   #Bert 词嵌入矩阵
            ...
            self.tags = tf.placeholder(tf.int32, [None, None], name="tags")  #标签tag
            self.is_training = tf.placeholder(tf.bool, [])  #是否训练
            self.learning_rate = tf.placeholder(tf.float32, [])  #学习率

            if args.including_charseqs:   #包括字序列
                self.form_charseqs = tf.placeholder(tf.int32, [None, None], name="form_charseqs")  #字序列
                self.form_charseq_lens = tf.placeholder(tf.int32, [None], name="form_charseq_lens") #字序列长度
                self.form_charseq_ids = tf.placeholder(tf.int32, [None,None], name="form_charseq_ids") #字序列ID
            ...
            # Trainable embeddings for forms 对于词的嵌入矩阵 [num_forms, args.we_dim]
            form_embeddings = tf.get_variable("form_embeddings", shape=[num_forms, args.we_dim], dtype=tf.float32)
            inputs.append(tf.nn.embedding_lookup(form_embeddings, self.form_ids)) # Looks up ids in a list of embedding tensors.

            # Trainable embeddings for lemmas 对于词干的嵌入矩阵 [num_lemmas, args.we_dim]
            ...
            # POS encoded as one-hot vectors 位置信息使用one-hot编码
            inputs.append(tf.one_hot(self.pos_ids, num_pos))

            # Pretrained embeddings for forms 对于form的嵌入矩阵
            ...
            # Character-level form embeddings 字级别的嵌入矩阵 [num_form_chars, args.cle_dim],
            ...
            # Concatenate inputs 对于一个二维矩阵，第0个维度代表最外层方括号所框下的子集，第1个维度代表内部方括号所框下的子集。维度越高，括号越小。
            ...
            for i in range(args.rnn_layers):  # containing the forward and the backward final states of bidirectional rnn.
            ...
            #sequence_lengths: 该参数 是主要说明的，英文 直译过来就是：包括实际序列长度，形状为[batch_size] 的向量。下面会详细说的  sequence_lengths是一个向量,
            #transition_params:状态转移矩阵
            if args.decoding == "CRF": # conditional random fields
                output_layer = tf.layers.dense(hidden_layer_dropout, num_tags)  #全连接层 信息拟合  相当于添加一个层  输出的维度大小num_tag，改变inputs的最后一维
                # outputs = activation(inputs * kernel + bias)  weights matrix
                weights = tf.sequence_mask(self.sentence_lens, dtype=tf.float32)
                #sequence_lengths * max_seq_len 该矩阵 用来 计算 后续 损失时，将 无效 词和tag 去除。这里面的值 都
                ...
                """Computes the log-likelihood of tag sequences in a CRF.
                  ...
                  """
                loss = tf.reduce_mean(-log_likelihood)   #定义损失
    ...
    def train_epoch(self, train, learning_rate, args):
        pbar = tqdm(total=len(train.sentence_lens))
    ...
    def evaluate(self, dataset_name, dataset, args):
        ...
        if args.corpus in ["CoNLL_en", "CoNLL_de", "CoNLL_nl", "CoNLL_es"]:
            ...
            os.system("cd {} && sh ../../run_eval_nested.sh {} {}".format(args.logdir, dataset_name, os.path.dirname(args.__dict__[dataset_name + "_data"])))
    ...
    def predict(self, dataset_name, dataset, args, prediction_file, evaluating=False):
        if evaluating:
            self.session.run(self.reset_metrics)
        tags = []
        pbar1 = tqdm(total=len(train.sentence_lens))
        while not dataset.epoch_finished():
            pbar1.update(args.batch_size)
            seq2seq = args.decoding == "seq2seq"
        ...
        for s in range(len(forms)):  #句子长度
            j = 0
            for i in range(len(forms[s])):  #词的长度
                if args.decoding == "seq2seq": # collect all tags until <eow>
                    ...
                    while j < len(tags[s]) and dataset.factors[dataset.TAGS].words[tags[s][j]] != "<eow>":
                        labels.append(dataset.factors[dataset.TAGS].words[tags[s][j]])  #预测出的标签
                        j += 1
                    j += 1 # skip the "<eow>"
    ...
    def predict_to_file(self,args):
        """预测结果到文件，方便提交
        """
        fw = open("ner_pred_bert.json", 'w', encoding='utf-8')
        with open("{}/{}_system_predictions.conll".format(args.logdir, "test")) as fr:
            event_list = []
            role = []
            arguments = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bert_embeddings_dev", default="test_run/dev_word_embedding_s.txt", type=str, help="Pretrained BERT embeddings for dev data.")
    parser.add_argument("--bert_embeddings_test", default="test_run/test_word_embedding_s.txt", type=str, help="Pretrained BERT embeddings for test data.")
    parser.add_argument("--bert_embeddings_train", default="test_run/train_word_embedding_s.txt", type=str, help="Pretrained BERT embeddings for train data.")
    parser.add_argument("--beta_2", default=0.98, type=float, help="Beta 2.")
    parser.add_argument("--corpus", default="ACE2004", type=str, help="CoNLL_en|CoNLL_de|CoNLL_nl|CoNLL_es|ACE2004|ACE2005|GENIA.")
    ...
    parser.add_argument("--dev_data", default="test_run/valid_data.txt", type=str, help="Dev data.")
    ...
    parser.add_argument("--epochs", default="20:1e-4", type=str, help="Epochs and learning rates.")
    ...
    parser.add_argument("--name", default="test_run", type=str, help="Experiment name.")
    parser.add_argument("--predict", default="logs/tagger.py-2020-05-14_183338-bs=6,be=1,c=ACE2004,d=CRF,ee=0,e=20:1e-4,fm=None,fe=0,fwm=0,lwm=0,n=test_run,p=None,rc=LSTM,rl=1",type=str, help="Predict using the passed model.")
    ...
    parser.add_argument("--test_data", default="test_run/test_data.txt", type=str, help="Test data.")
    parser.add_argument("--train_data", default="test_run/train_data.txt", type=str, help="Training data.")
    ...
    args.predict="logs/tagger.py-2020-05-14_183338-bs=6,be=1,c=ACE2004,d=CRF,ee=0,e=20:1e-4,fm=None,fe=0,fwm=0,lwm=0,n=test_run,p=None,rc=LSTM,rl=1"
    if args.predict:
