...
class MorphoDataset:
    ...
    - strings: Strings of the original words.  词串
    - word_ids: Word ids of the original words (uses <unk> and <pad>).  词ID
    - words_map: String -> word_id map. 词匹配
    - words: Word_id -> string list.   词
    - alphabet_map: Character -> char_id map.  字母表匹配
    - alphabet: Char_id -> character list.   字母表
    - charseq_ids: Character_sequence ids of the original words. 字序列ID
    - charseqs_map: String -> character_sequence_id map.   字序列匹配
    ...
    def __init__(self, filename, train=None, shuffle_batches=True, max_sentences=None, add_bow_eow=False, seq2seq=False, bert_embeddings_filename=None, flair_filename=None, elmo_filename=None):
                ...
                if line:
                    ...
                    for f in range(self.FACTORS):  #为什么在——FACTORS 寻找.一共有4个元素
                        ...
                        for word in words:  #每一个元素
                            ...
                            # Character-level information   字级信息
                            if word not in factor.charseqs_map:  #如果是个新词
                                factor.charseqs_map[word] = len(factor.charseqs)
                                factor.charseqs.append([])
                                if add_bow_eow:
                                    factor.charseqs[-1].append(factor.alphabet_map['<bow>'])
                                for c in word: #每个字符级别的标记信息
                                    if c not in factor.alphabet_map:
                                        if train:  #如果是训练过程那就是 unk
                                            c = '<unk>'
                                        else:    #如果是过程那就是 id
                                            factor.alphabet_map[c] = len(factor.alphabet)
                                            factor.alphabet.append(c)
                                    factor.charseqs[-1].append(factor.alphabet_map[c]) #加入
                                if add_bow_eow:
                                    factor.charseqs[-1].append(factor.alphabet_map['<eow>'])
                            factor.charseq_ids[-1].append(factor.charseqs_map[word])  #加入每个字符的id

                            # Word-level information 词级信息
        ...
        # Compute sentence lengths 计算句子长度
        sentences = len(self._factors[self.FORMS].word_ids)  #token 级别的长度
        ...
        # Load pretrained BERT embeddings 加载词嵌入
    ...
    def _next_batch(self, batch_perm, form_wes_model, lemma_wes_model, fasttext_model, including_charseqs, seq2seq=False):
        ...
        if self._bert_embeddings:
            ...
            for i in range(batch_size):
                batch_bert_embeddings[i, :self._bert_embeddings[batch_perm[i]].shape[0]-1] = self._bert_embeddings[batch_perm[i]][0:-1]
