...
cat ${name}_system_predictions.conll | python $(dirname $0)/conll2eval_nested.py > ${name}_system_entities.txt
python $(dirname $0)/compare_nested_entities.py $(dirname $0)/${gold_dir}/${name}_gold_entities.txt ${name}_system_entities.txt > ${name}.eval
